/*
Nhat Nguyen 4/18/2012
Don't user "live" event of jQuery instead of using plugin livequery
*/
...
(function ($, document, window) {
        ...
        if (!closing) {
            ...
            if (!open) {
                ...
                $overlay.css({
                    ...
                    "filter": "alpha(opacity =" + settings.opacity * 100 + ")", /*Fix opacity for IE*/
    ...
    function addBindings() {
        if ($box) {
            if (!init) {
                ...
                if (jQuery().livequery) {
                    $('.' + boxElement, document).livequery('click', function (e) {
    ...
    publicMethod.close = function () {
        if (open && !closing) {
            ...
            // keeps the top and left positions within the browser's viewport.
            if (settings.right !== false) {
                left += Math.max($window.width() - settings.initialWidth - interfaceWidth - setSize(settings.right, 'x'), 0);
            } else if (settings.left !== false) {
                left += setSize(settings.left, 'x');
            } else {
                left += Math.round(Math.max($window.width() - settings.initialWidth - interfaceWidth, 0) / 2);
            }

            if (settings.bottom !== false) {
                top += Math.max($window.height() - settings.h - settings.initialHeight - setSize(settings.bottom, 'y'), 0);
            } else if (settings.top !== false) {
                top += setSize(settings.top, 'y');
            } else {
                top += Math.round(Math.max($window.height() - settings.initialHeight - interfaceHeight, 0) / 2);
            }
...
(function ($, document, window) {
    ...
    // Abstracting the HTML and event identifiers for easy rebranding
	overcolorbox = 'overcolorbox', prefix = 'overcbox', boxElement = prefix + 'Element',
    ...
    function makeSettings() {
        var i;
        settings = $.extend({}, $.data(element, overcolorbox));
    ...
    function launch(target) {
        if (!closing) {
            ...
            if (settings.rel !== 'nofollow') {
                $related = $('.' + boxElement).filter(function () {
                    var relRelated = $.data(this, overcolorbox).rel || this.rel;
                    return (relRelated === settings.rel);
                });
                index = $related.index(element);
    ...
    publicMethod = $.fn[overcolorbox] = $[overcolorbox] = function (options, callback) {
        var $this = this;
        options = options || {};
        ...
        if (addBindings()) {
            ...
            $this.each(function () {
                $.data(this, overcolorbox, $.extend({}, $.data(this, overcolorbox) || defaults, options));
            }).addClass(boxElement);
        ...
        callback = function () {
            ...
            if (total > 1) {// handle grouping
                ...
                if (settings.preloading) {
                    preload = [getIndex(-1), getIndex(1)];
                    while (i = $related[preload.pop()]) {
                        src = $.data(i, overcolorbox).href || i.href;
                        if ($.isFunction(src)) {
                            src = src.call(i);
                        }
                        if (isImage(src)) {
    ...
    publicMethod.remove = function () {
        $([]).add($box).add($overlay).remove();
        $box = null;
        $('.' + boxElement).removeData(overcolorbox).removeClass(boxElement).die();
    };
    // A method for fetching the current element ColorBox is referencing.
    // returns a jQuery object.
    publicMethod.element = function () {
...
(function ($, document, window) {
    ...
    // Abstracting the HTML and event identifiers for easy rebranding
	undercolorbox = 'undercolorbox', prefix = 'undercbox', boxElement = prefix + 'Element',
    ...
    function makeSettings() {
        var i;
        settings = $.extend({}, $.data(element, undercolorbox));
    ...
    function launch(target) {
        if (!closing) {
            ...
            if (settings.rel !== 'nofollow') {
                $related = $('.' + boxElement).filter(function () {
                    var relRelated = $.data(this, undercolorbox).rel || this.rel;
                    return (relRelated === settings.rel);
                });
                index = $related.index(element);
    ...
    publicMethod = $.fn[undercolorbox] = $[undercolorbox] = function (options, callback) {
        var $this = this;
        options = options || {};
        ...
        if (addBindings()) {
            ...
            $this.each(function () {
                $.data(this, undercolorbox, $.extend({}, $.data(this, undercolorbox) || defaults, options));
            }).addClass(boxElement);
        ...
        callback = function () {
            ...
            if (total > 1) {// handle grouping
                ...
                if (settings.preloading) {
                    preload = [getIndex(-1), getIndex(1)];
                    while (i = $related[preload.pop()]) {
                        src = $.data(i, undercolorbox).href || i.href;
                        if ($.isFunction(src)) {
                            src = src.call(i);
                        }
                        if (isImage(src)) {
    ...
    publicMethod.remove = function () {
        $([]).add($box).add($overlay).remove();
        $box = null;
        $('.' + boxElement).removeData(undercolorbox).removeClass(boxElement).die();
    };
    // A method for fetching the current element ColorBox is referencing.
    // returns a jQuery object.
    publicMethod.element = function () {
